...
export default function UserManagement() {
  ...
  if (loading) return <div className="p-24 text-accent animate-pulse">SYNCHRONIZING_USER_DATABASE...</div>;
  ...
  return (
    <main className="min-h-screen bg-background font-mono p-24">
      ...
      <div className="flex justify-between items-center mb-12">
        <h1 className="text-3xl font-bold text-accent crt-glow flex items-center gap-4">
          <UserCog className="w-8 h-8" /> USER_AUTHORIZATION_PROTOCOL
        </h1>
        <button
          onClick={fetchUsers}
          className={`px-4 py-2 border border-accent/50 text-accent text-sm hover:bg-accent/10 transition-all ${isRefreshing ? 'animate-pulse' : ''}`}
        >
          [REFRESH_DATABASE]
        </button>
      </div>

      <div className="border border-accent/20 bg-background/40 rounded-lg overflow-hidden">
        <table className="w-full text-left">
          <thead className="bg-accent/10 text-accent border-b border-accent/20">
            <tr>
          ...
          <tbody>
            {users.length === 0 ? (
              ...
              users.map((u) => (
                <tr key={u.id} className="border-b border-accent/10 hover:bg-white/5 transition-colors">
                  <td className="p-4 font-mono text-xs">{u.id}</td>
                  <td className="p-4">
                    <span className={`px-2 py-1 rounded text-[10px] font-bold ${
                      u.role === 'owner' ? 'bg-accent/20 text-accent' :
                      u.role === 'family' ? 'bg-accent/20 text-accent' :
      ...
      <div className="mt-12 p-6 border border-white/10 bg-white/5 rounded-md">
        <h2 className="text-xl font-bold mb-4 flex items-center gap-2">
          <UserIcon className="w-5 h-5 text-accent" /> PUBLIC_ACCESS_RULES
        </h2>
